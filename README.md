...
services:
  ...
  app:
    build: .
    command: sh -c "wait-for postgres:5432 && python manage.py collectstatic --no-input && python manage.py migrate && gunicorn tracker.wsgi -b 0.0.0.0:8000"
    container_name: app
    depends_on:
      - postgres
      - rabbitmq
  ...
  celery_worker:
    command: sh -c "wait-for rabbitmq:5672 && wait-for app:8000 -- celery -A tracker worker -l info"
  ...
  celery_beat:
    command: sh -c "wait-for rabbitmq:5672 && wait-for app:8000 -- celery -A tracker beat -l info --scheduler django_celery_beat.schedulers:DatabaseScheduler"
...
```bash
export DJANGO_SETTINGS_MODULE=tracker.settings.production
...
Firstly, the Celery app needs to be defined in [`mysite/celery_app.py`](tracker/celery.py),
...
CELERY_BEAT_SCHEDULE = {
    'queue_every_five_mins': {
        'task': 'vaccine_tracker.tasks.query_every_five_mins',
...
usually in files named `tasks.py` by convention. The [`polls/tasks.py`](vaccine_tracker/tasks.py) file
contains the following (very contrived!) tasks

```python
# vaccine_tracker/tasks.py
...
these view functions from [`polls/views.py`](vaccine_tracker/views.py)

```python
# vaccine_tracker/views.py
...
def index(request):
    ...
    return render(request, "vaccine_tracker/index.html", context)
...
wait-for postgres:5432\
  ...
  && gunicorn tracker.wsgi -b 0.0.0.0:8000
```
